...
const menuItems = [
  ...
  // The profile route will be handled separately below
];
...
export default function SideMenu({ open, onClose, user, currentPage }) {
  ...
  // Determine the correct profile route
  const profileRoute = user?.type === 'institution' ? '/instituicao/perfil' : '/conta';
  return (
    <>
      ...
      <aside className={`side-menu${open ? ' open' : ''}`}>
        ...
        <nav className="side-menu-nav">
          ...
          {/* Profile link with dynamic route */}
          <button
            className={`side-menu-link${currentPage === profileRoute ? ' active' : ''}`}
            onClick={() => { navigate(profileRoute); onClose(); }}
          >
            <span className="side-menu-link-icon">
              <svg width="24" height="24" fill="none" viewBox="0 0 24 24"><circle cx="12" cy="8" r="4" stroke="#222" strokeWidth="2"/><path d="M4 20c0-2.21 3.582-4 8-4s8 1.79 8 4" stroke="#222" strokeWidth="2"/></svg>
            </span>
